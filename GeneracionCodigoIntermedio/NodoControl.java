		...
		switch(this.getLex()) {
			...
			case "OUTF":
				...
				//if(simbolo.getUso().equals("cadena")) {
					//salida+= "invoke MessageBox, NULL, addr " + simbolo.getLexema().replace("[", "").replace("]", "").replace(" ","_").replace(".","_").replace(" \n", "_")  +", addr " +simbolo.getLexema().replace("[", "").replace("]", "").replace(".","_").replace(" ","_") +", MB_OK \n";
					...
					if (simbolo != null ) {
				        ...
				        // Caso 2: Identificador (Variable)
				            ...
				            else if (simbolo.getTipo().equals("DOUBLE")) {
				    ...
				    // Caso 3: Expresión Compleja
				        ...
				        if (simbolo1.getTipo().equals("INTEGER")) {
				            salida += "MOV AX, " + valorResultado + " \n";  // Mover el valor al registro AX
				            salida += "invoke wsprintf, addr auxCadena, addr msg , AX \n";  // Convertir a texto
				            salida += "invoke MessageBox, NULL, addr auxCadena, addr auxCadena, MB_OK \n";  // Mostrar
				        } else if (simbolo1.getTipo().equals("DOUBLE")) {
				...
				/*	 ; Obtener la parte entera
					    ...
					    fstcw [fpu_cw]          ; Guardar el control word
					    mov ax, [fpu_cw]
					    and ax, 0F3FFh          ; Limpiar bits de control de redondeo
					    or ax, 0C00h            ; Establecer redondeo hacia cero
					    mov [fpu_cw], ax
					    fldcw [fpu_cw]          ; Cargar nuevo control word
					    frndint                 ; Redondear al entero más cercano
					    fistp [intPart]         ; Guardar la parte entera
					    
					    ; Obtener la parte decimal
					    fld QWORD PTR [result]
					    fisub [intPart]         ; Restar la parte entera
					    fmul QWORD PTR [factor] ; Multiplicar por 100 para obtener 2 decimales
					    fistp [decPart]         ; Guardar la parte decimal

					    ; Mostrar en MessageBox
					    invoke wsprintf, addr buffer, addr msg, [intPart], [decPart]
					    invoke MessageBox, NULL, addr buffer, addr msg, MB_OK*/
