		...
		public String getAssembler() throws IOException {
            ...
            case ":=":
				...
				if(TablaDeSimbolos.obtenerSimbolo(hijoIzq).getTipo().equals("INTEGER")){
					...
					if (!getDer().getLex().equals("ejecucion funcion")) {
					salida+= "MOV AX , " + hijoDer + "\n"; 
					}
					salida+= "MOV " + hijoIzq + ", AX"+ "\n";	
				}else if(TablaDeSimbolos.obtenerSimbolo(hijoIzq).getTipo().equals("DOUBLE")) {
					if(TablaDeSimbolos.obtenerSimbolo(hijoDer).getTipo().equals("INTEGER") ||  getDer().getLex().equals("CONVERSION") ) {
					hijoIzq= auxI;
					hijoDer= auxD;
					hijoIzq= contieneAux(hijoIzq);
					hijoDer= contieneAux(hijoDer);
					//salida += "FILD " + hijoDer + "\n";
					if (!getDer().getLex().equals("ejecucion funcion") && !getDer().getLex().equals("CONVERSION") ) {
						salida+= "FLD " + hijoDer + "\n";  
						}
					salida+= "FSTP " + hijoIzq + "\n";
            ...
            case "*":  
            	...
            	}else if(TablaDeSimbolos.obtenerSimbolo(hijoIzq).getTipo().equals("DOUBLE") || getIzq().getLex().equals("CONVERSION")){
            		hijoIzq= contieneAux(hijoIzq);
					hijoDer= contieneAux(hijoDer);
					ArbolSintactico.indiceAux++;
					if(!getIzq().getLex().equals("CONVERSION") && !getDer().getLex().equals("CONVERSION")) {
	            		salida+= "FLD " +hijoIzq+ "\n";
	        			salida+= "FMUL " +hijoDer+ "\n";
	            		salida+= "FSTP @aux" +ArbolSintactico.indiceAux +"\n";
	            		}else {
	            			if(getDer().getLex().equals("CONVERSION")) {
	            			salida+= "FLD " +hijoIzq+ "\n";
	            			salida+= "FMUL " + "\n";
	                		salida+= "FSTP @aux" +ArbolSintactico.indiceAux +"\n";
	            			}else {
            ...
            case "Cuerpo_Fun":
            	 ...
            	 Simbolo s1 = TablaDeSimbolos.obtenerParametro2(getIzq().getLex());
            	 
            	 hijoIzq = getIzq().getLex();
            	 salida += hijoIzq  + ": \n";
            	 salida += "PUSH EBP" + "\n";   //Guardar el valor actual de EBP\r\n"
            	 salida += "MOV EBP, ESP " + "\n" ;   //     Configurar el marco de pila\r\n"
            	 if (s1.getTipo().equals("INTEGER")) {
            	
            	  String salder = getDer().getAssembler();
            	  String param = contieneAux(s1.getLexema());
            	  salder = salder.replace(param,"[EBP+8]" ) ;
            	  salida += salder;
            	 }else {
            		 String salder = getDer().getAssembler();
               	  String param = contieneAux(s1.getLexema());
            ...
            case "ejecucion funcion":
				...
				hijoDer= contieneAux(hijoDer);
				if (TablaDeSimbolos.obtenerSimboloFuncion(getIzq().getLex()).getTipo().equals("INTEGER")) {
				     salida += "MOV AX ,"+ hijoDer +"\n";
				/*salida+= "MOV @aux"+ArbolSintactico.indiceAux+ " , AX " + "\n";
        		ArbolSintactico.pilaAuxs.push(ArbolSintactico.indiceAux);
        		TablaDeSimbolos.agregarSimbolo("@aux"+ArbolSintactico.indiceAux, ConstantesCompilador.ID);
            	...
            	salida += "ADD ESP, 4 " + " \n";//limpiar pila
					...
					salida += "FLD " + hijoDer + "\n";     // Argumento flotante
			        salida += "CALL " + hijoIzq + "\n";
			        salida += "ADD ESP, 8" + "\n";         // Limpiar pila (8 bytes para 64 bits)
            ...
            case "RET": 
            	...
            	Simbolo simboloRetorno = TablaDeSimbolos.obtenerSimbolo(hijoIzq);
                if (simboloRetorno.getTipo().equals("INTEGER")) {
                	hijoIzq = contieneAux(hijoIzq);
                    salida += "MOV AX, " + hijoIzq + "\n"; // Retorno para enteros (16 bits)
                } else if (simboloRetorno.getTipo().equals("DOUBLE")) {
                	hijoIzq = contieneAux(hijoIzq);
                    salida += "FLD qword ptr " + hijoIzq + "\n";     // Retorno para flotantes (64 bits)
                }
            	//salida +="MOV AX , "+ hijoIzq +" \n"  ;
            	salida += "POP EBP"+ "\n";
            	salida += "ret  \n";
